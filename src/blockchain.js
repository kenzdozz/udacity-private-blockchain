...
class Blockchain {
    ...
    submitStar(address, message, signature, star) {
        ...
        return new Promise(async (resolve, reject) => {
            const [walletAddress, time] = message.split(':');
            if (walletAddress !== address) {
                return reject(Error('Invalid address'));
            }
            const messageTime = parseInt(time);
            const currentTime = parseInt(new Date().getTime().toString().slice(0, -3));
            if (currentTime - messageTime > 5 * 60) {
                return reject(Error('Invalid message'));
            }
    ...
    validateChain() {
        ...
        return new Promise(async (resolve) => {
            for (let i = 0; i < self.chain.length; i++) {
                ...
                if (prevBlock && prevBlock.hash !== block.previousBlockHash) {
                    errorLog.push(Error(`Block not valid.`));
                }
            }
